...
Here’s a production-ready, **one-shot Bash script** to install and configure the latest **SonarQube Server LTA (2025.1)** on **Amazon Linux 2023** with **Amazon Corretto 21** and **PostgreSQL 15**. It creates a dedicated `sonarqube` user, applies the required kernel/ulimit settings, installs SonarQube under `/opt/sonarqube`, wires up a systemd service, and starts it on port **9000**.

> Notes
>
> * SonarQube 2025.1 LTA supports **Java 21**; this script uses **Corretto 21**. ([SonarSource][1], [SonarSource Documentation][2])
> * Host sizing (small): ≥4 GB RAM, 2 vCPU (baseline guidance). ([SonarSource Documentation][3])
> * If you prefer **Amazon RDS** for PostgreSQL, set the DB variables below to your RDS endpoint and skip the local PostgreSQL block (it’s guarded with a toggle).

---

### Install script (save as `install-sonarqube-al2023.sh`)
...
```bash
chmod +x install-sonarqube-al2023.sh
sudo ./install-sonarqube-al2023.sh
```

**Customizations** (examples):

```bash
# Use external RDS instead of local Postgres
export USE_LOCAL_PG=false
export DB_HOST="your-rds-endpoint.rds.amazonaws.com"
export DB_PORT="5432"
export DB_NAME="sonarqube"
export DB_USER="sonar"
export DB_PASS="super-strong-password"
sudo ./install-sonarqube-al2023.sh

# Pin a specific SonarQube build in the 2025.1 line if needed
export SONAR_VERSION="2025.1.2"     # example patch
sudo ./install-sonarqube-al2023.sh
```
...
* **Security Group**: allow inbound `TCP 9000` from your IP.
* **First login**: `admin / admin` → change password.
* **Health check**: `sudo journalctl -u sonarqube -e --no-pager` and check `${SONAR_DATA}/logs/*.log`.
* **Scanners**: Use Scanner versions compatible with 2025.x; ensure Java on build agents per your language/CI. ([SonarSource Documentation][4])

**Why these versions?**
SonarQube’s 2025.1 **LTA** is the current long-term active release (recommended for stability), and it supports **Java 21 LTS** on the server. Amazon Linux 2023 provides **Corretto 21** and **PostgreSQL 15** packages via `dnf`, which align with SonarQube’s requirements. ([SonarSource][1], [SonarSource Documentation][2], [Stack Overflow][5])

If you want this tailored for **Amazon Linux 2** (slightly different package names/commands) or converted to a **Docker Compose** deployment, say the word and I’ll drop a variant.

[1]: https://www.sonarsource.com/products/sonarqube/downloads/?utm_source=chatgpt.com "Download SonarQube"
[2]: https://docs.sonarsource.com/sonarqube-server/2025.1/server-upgrade-and-maintenance/release-notes-and-notices/lta-to-lta-release-notes/?utm_source=chatgpt.com "LTA to LTA release notes"
[3]: https://docs.sonarsource.com/sonarqube-server/10.8/setup-and-upgrade/installation-requirements/server-host/?utm_source=chatgpt.com "SonarQube Server host requirements and recommendations"
[4]: https://docs.sonarsource.com/sonarqube-server/10.6/?utm_source=chatgpt.com "SonarQube 10.6 Documentation"
[5]: https://stackoverflow.com/questions/72307187/how-to-install-postgresql-client-to-amazon-ec2-linux-machine?utm_source=chatgpt.com "How to install postgresql-client to Amazon EC2 Linux ..."
